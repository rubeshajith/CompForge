...
function icon(name: string, style?: CSSProperties) {
  return (
    <span aria-hidden="true" className="material-symbols-outlined" style={style}>
      {name}
    </span>
  );
}

export function EcommerceDashboardPreview({ config }: EcommerceDashboardPreviewProps) {
  const cssVars = {
  ...
  return (
    <div className="edash" style={cssVars}>
      ...
      <aside className="edash__sidebar">
        ...
        <button className="edash__primary-btn">{icon("add")} New Dashboard</button>
        ...
        <div className="edash__sidebar-footer">
          <a className="edash__nav-item" href="#">{icon("settings")}<span>Settings</span></a>
          <a className="edash__nav-item" href="#">{icon("help")}<span>Support</span></a>
        </div>
      </aside>
      ...
      <header className="edash__topbar">
        ...
        <div className="edash__top-actions">
          <button>{icon("notifications")}<i /></button>
      ...
      <main className="edash__main">
        <div className="edash__page-head">
          ...
          <div className="edash__range">
            ...
            <button className="edash__outline-btn">{icon("calendar_today")} Custom Range</button>
          </div>
        </div>

        <section className="edash__metrics">
          <MetricCard iconName="payments" label="Daily Gross Revenue" value="$48,290" change="+12.5%" tone="accent" progress="75%" />
          <MetricCard iconName="conversion_path" label="Conversion Rate" value="4.82%" change="+2.1%" tone="success" note="Global average: 2.3%" />
          <MetricCard iconName="shopping_cart_checkout" label="Abandoned Carts" value="1,402" change="-4.2%" tone="danger" progress="65%" />
          <div className="edash__metric edash__metric--alert">
            ...
            <div>
              ...
              <h3>14</h3>
              <button className="edash__link-btn">Review Inventory {icon("arrow_forward")}</button>
            </div>
          </div>
        </section>
        ...
        <section className="edash__middle">
          ...
          <div className="edash__card edash__map-card">
            ...
            <div className="edash__regions">
              <p><span>USA</span><strong>$124,000</strong></p>
              <p><span>Germany</span><strong>$82,300</strong></p>
              <p><span>Japan</span><strong>$54,100</strong></p>
            </div>
          </div>
        </section>
        ...
        <section className="edash__table-card">
          ...
          <div className="edash__table-wrap">
            <table>
              <thead>
                <tr>
                  <th>Product Info</th><th>SKU</th><th>Price</th><th>Sold</th><th>Stock Level</th><th>Trend</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  <tr key={product.sku}>
                    <td>
                      <div className="edash__product">
                        <img alt="" className="edash__thumb" src={product.image} />
                    ...
                    <td>
                      <div className="edash__stock">
                        <div><i style={{ width: product.stockValue, background: product.color === "warning" ? "var(--ed-warning)" : "var(--ed-success)" }} /></div>
                        <strong style={{ color: product.color === "warning" ? "var(--ed-warning)" : "var(--ed-success)" }}>{product.stock}</strong>
                      </div>
                    </td>
                    <td>
                      <span className="edash__trend">{icon(product.trendIcon)} {product.trend}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <div className="edash__toast">{icon("check_circle")}<strong>New Sale! +$249.00 from New York, US</strong></div>
    </div>
  );
}
...
function MetricCard({
  ...
  const toneColor =
    tone === "success" ? "var(--ed-success)" : tone === "danger" ? "var(--ed-danger)" : "var(--ed-accent)";

  return (
    <div className="edash__metric">
      <div className="edash__metric-top">
        <div className="edash__metric-icon" style={{ color: toneColor }}>{icon(iconName)}</div>
        <span className="edash__change" style={{ color: toneColor }}>
          {icon(change.startsWith("-") ? "trending_down" : "trending_up")} {change}
        </span>
      </div>
      <div>
        <p>{label}</p>
        <h3>{value}</h3>
        {progress ? <div className="edash__progress"><i style={{ width: progress, background: toneColor }} /></div> : null}
