...
export function generateEmojiTSX(config: EmojiConfig): string {
  const {
    variant,
    animation,
    size,
...
export default function Emoji({ onAnimationCycle }: EmojiProps = {}) {
  ...
  const totalSize: number = ${totalSize};
...
export function generateEmojiTailwind(config: EmojiConfig): string {
  const {
    variant,
    animation,
    size,
  ...
  const currentKeyframe = animKeyframes[animation] ?? animKeyframes.bounce;
  const currentAnimName = animNameMap[animation] ?? "emojiBounce";
...
// Baked-in CSS variable tokens — update these to reskin the Emoji
const emojiVars: CSSProperties = {
  "--emoji-bg":         "${showBackground ? backgroundColor : "transparent"}",
  "--emoji-radius":     "${backgroundRadius}%",
} as CSSProperties;
...
export default function Emoji({ onAnimationCycle }: EmojiProps = {}) {
  ...
  return (
    ...
    >
      ${
        animationEnabled
          ? `<style>{\`${currentKeyframe}
      .emoji-animated {
        animation: ${currentAnimName} ${animationDuration}ms ease-in-out infinite;
      }
      \`}</style>`
          : ""
      }
      <svg
        ref={svgRef}
        className="${animationEnabled ? "emoji-animated" : ""} block"
        viewBox="0 0 100 100"
        width={${size}}
        height={${size}}
        xmlns="http://www.w3.org/2000/svg"
