...
function tsxPulseWave(c: LoaderConfig): string {
  const w = c.size;
  const h = Math.round(c.size * 0.75);
  const dur = (1.4 * c.speed).toFixed(2);
  return `  <div className="ldr-wrap">
...
function tsxHexBloom(c: LoaderConfig): string {
  return `  <div className="ldr-wrap">
    <div className="ldr ldr--hex-bloom">
      <div className="ldr__hex"></div>
      <div className="ldr__hex"></div>
...
function tsxOrbitRing(c: LoaderConfig): string {
  return `  <div className="ldr-wrap">
    <div className="ldr ldr--orbit-ring"></div>${c.showLabel ? `\n    <span className="ldr__label">${c.labelText}</span>` : ""}
  </div>`;
}

function tsxYinSpiral(c: LoaderConfig): string {
  return `  <div className="ldr-wrap">
    <div className="ldr ldr--yin-spiral"><div></div></div>${c.showLabel ? `\n    <span className="ldr__label">${c.labelText}</span>` : ""}
  </div>`;
}

function tsxBarFill(c: LoaderConfig): string {
  return `  <div className="ldr-wrap">
    <div className="ldr ldr--bar-fill"></div>${c.showLabel ? `\n    <span className="ldr__label">${c.labelText}</span>` : ""}
  </div>`;
}

function tsxSlideShift(c: LoaderConfig): string {
  return `  <div className="ldr-wrap">
    <div className="ldr ldr--slide-shift">
      <div></div>
      <div></div>
...
function tsxDotBounce(c: LoaderConfig): string {
  return `  <div className="ldr-wrap">
    <div className="ldr ldr--dot-bounce">
      <div></div><div></div><div></div>
    </div>${c.showLabel ? `\n    <span className="ldr__label">${c.labelText}</span>` : ""}
...
function tsxMorphBlob(c: LoaderConfig): string {
  return `  <div className="ldr-wrap">
    <div className="ldr ldr--morph-blob"></div>${c.showLabel ? `\n    <span className="ldr__label">${c.labelText}</span>` : ""}
  </div>`;
}

function tsxStaggerBars(c: LoaderConfig): string {
  return `  <div className="ldr-wrap">
    <div className="ldr ldr--stagger-bars">
      <div></div><div></div><div></div><div></div><div></div>
    </div>${c.showLabel ? `\n    <span className="ldr__label">${c.labelText}</span>` : ""}
...
const TSX_GENERATORS: Record<LoaderVariant, (c: LoaderConfig) => string> = {
  "pulse-wave": tsxPulseWave,
  "hex-bloom": tsxHexBloom,
  "orbit-ring": tsxOrbitRing,
  "yin-spiral": tsxYinSpiral,
  "bar-fill": tsxBarFill,
  "slide-shift": tsxSlideShift,
  "dot-bounce": tsxDotBounce,
  "morph-blob": tsxMorphBlob,
  "stagger-bars": tsxStaggerBars,
};

// ─── TSX + CSS ────────────────────────────────────────────────────────────────

export function generateLoaderTSX(config: LoaderConfig): string {
  const inner = TSX_GENERATORS[config.variant](config);
  return `import React from "react";
import "./Loader.css";
...
function twPulseWave(c: LoaderConfig): string {
  const w = c.size;
  const h = Math.round(c.size * 0.75);
  const dur = (1.4 * c.speed).toFixed(2);
  return `  <div className="inline-flex flex-col items-center justify-center gap-3 rounded-xl p-8 bg-[var(--ldr-bg)]">
    <svg className="ldr ldr--pulse-wave" width="${w}" height="${h}" viewBox="0 0 64 48">
      <polyline className="ldr__back" points="0.157 23.954, 14 23.954, 21.843 48, 43 0, 50 24, 64 24" />
      <polyline className="ldr__front" points="0.157 23.954, 14 23.954, 21.843 48, 43 0, 50 24, 64 24" />
    </svg>${c.showLabel ? `\n    <span className="font-mono text-[13px] tracking-widest text-[var(--ldr-label)]">${c.labelText}</span>` : ""}
    <style>{\`
      .ldr--pulse-wave .ldr__back,
      .ldr--pulse-wave .ldr__front {
        fill: none;
        stroke-width: ${c.strokeWidth};
        stroke-linecap: round;
      ...
      .ldr--pulse-wave .ldr__back  { stroke: var(--ldr-secondary); }
      .ldr--pulse-wave .ldr__front {
        stroke: var(--ldr-primary);
        stroke-dasharray: 48, 144;
        stroke-dashoffset: 192;
        animation: ldr-wave ${dur}s linear infinite;
      }
...
function twHexBloom(c: LoaderConfig): string {
  const s = c.size;
  const unit = s * 0.375;
  const half = unit / 2;
  const bodyH = unit * 0.5;
  ...
  const nthRules = positions
    ...
    .join("\n      ");
  return `  <div className="inline-flex flex-col items-center justify-center gap-3 rounded-xl p-8 bg-[var(--ldr-bg)]">
    <div className="ldr--hex-bloom relative" style={{ width: ${s}, height: ${s} }}>
      <div className="ldr__hex absolute"></div>
      <div className="ldr__hex absolute"></div>
      <div className="ldr__hex absolute"></div>
      <div className="ldr__hex absolute"></div>
      <div className="ldr__hex absolute"></div>
    ...
    <style>{\`
      ...
      .ldr--hex-bloom .ldr__hex::before {
        top: -${capH.toFixed(1)}px;
        border-bottom: ${capH.toFixed(1)}px solid var(--ldr-primary);
      }
      .ldr--hex-bloom .ldr__hex::after {
        bottom: -${capH.toFixed(1)}px;
        border-top: ${capH.toFixed(1)}px solid var(--ldr-primary);
      }
      .ldr--hex-bloom .ldr__hex > div { background: var(--ldr-primary); width: 100%; height: 100%; }
      ${nthRules}
      @keyframes ldr-hexbloom {
        0%,20%,80%,100% { opacity:0; transform:scale(0); }
        30%,70%          { opacity:1; transform:scale(1); }
...
function twOrbitRing(c: LoaderConfig): string {
  const s = c.size;
  const dot = Math.round(s * 0.19);
  const pad = Math.round(s * 0.047);
  const sw = c.strokeWidth;
  const dur = (2 * c.speed).toFixed(2);
  return `  <div className="inline-flex flex-col items-center justify-center gap-3 rounded-xl p-8 bg-[var(--ldr-bg)]">
    <div className="ldr--orbit-ring" style={{ width: ${s}, height: ${s} }}></div>${c.showLabel ? `\n    <span className="font-mono text-[13px] tracking-widest text-[var(--ldr-label)]">${c.labelText}</span>` : ""}
    <style>{\`
      .ldr--orbit-ring {
        border-radius: 50%;
        padding: ${pad}px;
        background:
          radial-gradient(farthest-side, var(--ldr-primary) 95%, transparent) 50% 0 / ${dot}px ${dot}px no-repeat,
          radial-gradient(farthest-side, transparent calc(100% - ${sw + 1}px), var(--ldr-primary) calc(100% - ${sw}px)) content-box;
        box-sizing: border-box;
        animation: ldr-orbit ${dur}s linear infinite;
      }
      @keyframes ldr-orbit { to { transform: rotate(1turn); } }
...
function twYinSpiral(c: LoaderConfig): string {
  const s = c.size;
  const dur = (2 * c.speed).toFixed(2);
  return `  <div className="inline-flex flex-col items-center justify-center gap-3 rounded-xl p-8 bg-[var(--ldr-bg)]">
    <div className="ldr--yin-spiral rounded-full overflow-hidden" style={{ width: ${s}, height: ${s}, background: 'var(--ldr-secondary)' }}>
      <div className="ldr--yin-spiral-inner w-full h-full rounded-full"></div>
    </div>${c.showLabel ? `\n    <span className="font-mono text-[13px] tracking-widest text-[var(--ldr-label)]">${c.labelText}</span>` : ""}
    <style>{\`
      .ldr--yin-spiral-inner {
        background:
          radial-gradient(154% 68.5% at top,    transparent 79.5%, var(--ldr-primary) 80%),
          radial-gradient(154% 68.5% at bottom, var(--ldr-primary) 79.5%, transparent 80%),
          radial-gradient(154% 68.5% at top,    transparent 79.5%, var(--ldr-primary) 80%);
        background-size: 50.5% 220%;
        background-position: -100% 0%, 0% 0%, 100% 0%;
        background-repeat: no-repeat;
...
function twBarFill(c: LoaderConfig): string {
  const s = c.size;
  const h = Math.round(s * 0.78);
  const dur = (2 * c.speed).toFixed(2);
  return `  <div className="inline-flex flex-col items-center justify-center gap-3 rounded-xl p-8 bg-[var(--ldr-bg)]">
    <div className="ldr--bar-fill" style={{ width: ${s}, height: ${h} }}></div>${c.showLabel ? `\n    <span className="font-mono text-[13px] tracking-widest text-[var(--ldr-label)]">${c.labelText}</span>` : ""}
    <style>{\`
      .ldr--bar-fill {
        -webkit-mask:
          no-repeat linear-gradient(90deg,#000 70%,transparent 0) calc(0*100%/4) 100%/calc(100%/5) calc(1*100%/5),
          ...
          no-repeat linear-gradient(90deg,#000 70%,transparent 0) calc(3*100%/4) 100%/calc(100%/5) calc(4*100%/5),
          no-repeat linear-gradient(90deg,#000 70%,transparent 0) calc(4*100%/4) 100%/calc(100%/5) calc(5*100%/5);
        background: linear-gradient(var(--ldr-primary) 0 0) left/0% 100% no-repeat var(--ldr-secondary);
        animation: ldr-barfill ${dur}s infinite steps(6);
      }
      @keyframes ldr-barfill { 100% { background-size: 120% 100%; } }
    \`}</style>
...
function twSlideShift(c: LoaderConfig): string {
  const s = Math.round(c.size * 0.625);
  const d = Math.round(s * 0.353);
  const travel = d * 1.5;
  const dur = (2 * c.speed).toFixed(2);
  return `  <div className="inline-flex flex-col items-center justify-center gap-3 rounded-xl p-8 bg-[var(--ldr-bg)]">
    <div className="ldr--slide-shift grid" style={{ width: ${s + d}, height: ${s + d} }}>
      <div className="ldr--slide-shift-item" style={{ gridArea: '1/1' }}></div>
      <div className="ldr--slide-shift-item" style={{ gridArea: '1/1' }}></div>
    </div>${c.showLabel ? `\n    <span className="font-mono text-[13px] tracking-widest text-[var(--ldr-label)]">${c.labelText}</span>` : ""}
    <style>{\`
      .ldr--slide-shift-item {
        clip-path: polygon(${d}px 0,100% 0,100% calc(100% - ${d}px),calc(100% - ${d}px) 100%,0 100%,0 ${d}px);
        background: conic-gradient(
          from -90deg at calc(100% - ${d}px) ${d}px,
          var(--ldr-secondary) 135deg, var(--ldr-primary) 0 270deg, color-mix(in srgb, var(--ldr-primary) 50%, transparent) 0
        );
        animation: ldr-slide ${dur}s infinite;
      }
      .ldr--slide-shift-item:last-child { animation-delay: -${c.speed.toFixed(2)}s; }
      @keyframes ldr-slide {
        0%   { transform: translate(0,0); }
        25%  { transform: translate(${travel.toFixed(1)}px,0); }
        50%  { transform: translate(${travel.toFixed(1)}px,${travel.toFixed(1)}px); }
...
function twDotBounce(c: LoaderConfig): string {
  const dot = Math.round(c.size * 0.22);
  const gap = Math.round(c.size * 0.12);
  const bounce = Math.round(c.size * 0.45);
  const dur = (1.2 * c.speed).toFixed(2);
  return `  <div className="inline-flex flex-col items-center justify-center gap-3 rounded-xl p-8 bg-[var(--ldr-bg)]">
    <div className="ldr--dot-bounce flex items-end" style={{ gap: ${gap}, height: ${c.size} }}>
      <div className="ldr--dot rounded-full bg-[var(--ldr-primary)]" style={{ width: ${dot}, height: ${dot} }}></div>
      <div className="ldr--dot rounded-full bg-[var(--ldr-primary)] [animation-delay:var(--ldr-d2)]" style={{ width: ${dot}, height: ${dot} }}></div>
      <div className="ldr--dot rounded-full bg-[var(--ldr-primary)] [animation-delay:var(--ldr-d3)]" style={{ width: ${dot}, height: ${dot} }}></div>
    </div>${c.showLabel ? `\n    <span className="font-mono text-[13px] tracking-widest text-[var(--ldr-label)]">${c.labelText}</span>` : ""}
    <style>{\`
      .ldr--dot {
        animation: ldr-bounce ${dur}s ease-in-out infinite;
...
function twMorphBlob(c: LoaderConfig): string {
  const s = c.size;
  const dur = (3 * c.speed).toFixed(2);
  return `  <div className="inline-flex flex-col items-center justify-center gap-3 rounded-xl p-8 bg-[var(--ldr-bg)]">
    <div className="ldr--morph-blob" style={{ width: ${s}, height: ${s}, background: \`radial-gradient(circle at 30% 30%, var(--ldr-primary), var(--ldr-secondary))\` }}></div>${c.showLabel ? `\n    <span className="font-mono text-[13px] tracking-widest text-[var(--ldr-label)]">${c.labelText}</span>` : ""}
    <style>{\`
      .ldr--morph-blob {
        animation: ldr-morph ${dur}s ease-in-out infinite;
      }
...
function twStaggerBars(c: LoaderConfig): string {
  const barW = Math.round(c.size * 0.14);
  const gap = Math.round(c.size * 0.08);
  const dur = (1.0 * c.speed).toFixed(2);
  const delays = [0, 0.1, 0.2, 0.1, 0];
  const nthRules = delays
    ...
    .join("\n      ");
  return `  <div className="inline-flex flex-col items-center justify-center gap-3 rounded-xl p-8 bg-[var(--ldr-bg)]">
    <div className="ldr--stagger-bars flex items-center" style={{ gap: ${gap}, height: ${c.size} }}>
      <div style={{ width: ${barW}, borderRadius: ${Math.round(barW / 2)} }}></div>
      <div style={{ width: ${barW}, borderRadius: ${Math.round(barW / 2)} }}></div>
      <div style={{ width: ${barW}, borderRadius: ${Math.round(barW / 2)} }}></div>
      <div style={{ width: ${barW}, borderRadius: ${Math.round(barW / 2)} }}></div>
      <div style={{ width: ${barW}, borderRadius: ${Math.round(barW / 2)} }}></div>
...
const TW_GENERATORS: Record<LoaderVariant, (c: LoaderConfig) => string> = {
  "pulse-wave": twPulseWave,
  "hex-bloom": twHexBloom,
  "orbit-ring": twOrbitRing,
  "yin-spiral": twYinSpiral,
  "bar-fill": twBarFill,
  "slide-shift": twSlideShift,
  "dot-bounce": twDotBounce,
  "morph-blob": twMorphBlob,
  "stagger-bars": twStaggerBars,
};

// ─── TSX + Tailwind ───────────────────────────────────────────────────────────

export function generateLoaderTailwind(config: LoaderConfig): string {
  const inner = TW_GENERATORS[config.variant](config);
  // dot-bounce delays need to be pre-computed for the CSS vars approach
  const d2 = (0.16 * config.speed).toFixed(2);
  const d3 = (0.32 * config.speed).toFixed(2);
...
const ldrVars: CSSProperties = {
  "--ldr-primary":   "${config.primaryColor}",
  "--ldr-secondary": "${config.secondaryColor}",
  "--ldr-bg":        "${config.backgroundColor}",
  "--ldr-label":     "${config.labelColor}",
  "--ldr-d2":        "${d2}s",
  "--ldr-d3":        "${d3}s",
} as CSSProperties;
