...
import { generateManufacturingDashboardCSS, generateManufacturingDashboardJSX } from "@/lib/generateManufacturingDashboardCode";

export default function ManufacturingDashboardPlayground() {
  const [mode, setMode] = useState<ManufacturingDashboardMode>("dark");
  const [config, setConfig] = useState<ManufacturingDashboardConfig>(darkManufacturingDashboardConfig);
  ...
  function handleModeToggle(newMode: ManufacturingDashboardMode) {
    setMode(newMode);
    setConfig((prev) => ({ ...manufacturingDashboardModePresets[newMode], ...preservedBehavior(prev) }));
  }

  function handleReset() {
    setConfig((prev) => ({ ...manufacturingDashboardModePresets[mode], ...preservedBehavior(prev) }));
  }

  const jsxCode = useMemo(() => generateManufacturingDashboardJSX(config), [config]);
  const cssCode = useMemo(() => generateManufacturingDashboardCSS(config), [config]);
  ...
  return (
    <ResizablePlayground
      ...
      controls={<ManufacturingDashboardControlPanel config={config} onChange={handleChange} onReset={handleReset} />}
