...
import {
  SaasDashboardConfig,
  darkSaasDashboardConfig,
} from "@/lib/saasDashboardConfig";
import {
  generateSaasDashboardCSS,
  generateSaasDashboardJSX,
  generateSaasDashboardTailwind,
  generateSaasDashboardTSX,
} from "@/lib/generateSaasDashboardCode";
...
export default function SaasDashboardPlayground() {
  const [config, setConfig] = useState<SaasDashboardConfig>(
    darkSaasDashboardConfig,
  );
  ...
  const tsxCode = useMemo(() => generateSaasDashboardTSX(config), [config]);
  const tailwindCode = useMemo(
    () => generateSaasDashboardTailwind(config),
    [config],
  );
  return (
    <ResizablePlayground
